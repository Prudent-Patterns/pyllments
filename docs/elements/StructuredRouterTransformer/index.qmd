...
`schema_output` is a **latched** port: the unified schema is emitted when ready and replayed to inputs connected later, so recipes can wire topology first without connection-time async side effects.
