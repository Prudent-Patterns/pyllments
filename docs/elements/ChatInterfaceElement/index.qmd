...
It provides the essential component of user interaction through receiving `MessagePayloads` and `ToolUsePayloads`, generating `MessagePayloads` as responses, and displaying tool-use lifecycle state from received `ToolUsePayloads`. Tool execution is handled by `ToolUseElement`, not the chat interface.
...
| `tool_use_emit_input`  | ToolUsePayload | Displays and then emits tool-use payloads via `tool_use_output`.                                 |
...
| `tool_message_output`     | MessagePayload          | Emits messages containing tool calls.                                                             |
| `tool_use_output`   | ToolUsePayload    | Emits processed tool-use payloads.                                                          |
