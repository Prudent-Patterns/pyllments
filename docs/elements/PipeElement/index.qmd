...
<span class="tab">Emits a payload from a sync context (no running event loop).</span>

```python
async_send_payload(payload: Any):
```

<span class="tab">Emits a payload and awaits ordered delivery. Use this inside async apps and tests.</span>
