...
1. When we type a message into the `ChatInterfaceElement` and hit send, in addition to rendering it in the chatfeed,it emits a `MessagePayload` through the `message_output` port.
    ...
    b. The `HistoryHandlerElement` receives a `MessagePayload` through the `payload_emit_input` port. This message is incorporated into the running history and can trigger context emission.
...
5. The assistant message is received by the `HistoryHandlerElement` in its `payload_emit_input` port. This triggers it to emit projected history on `context_output` to the `ContextBuilderElement`. Now, when we send a new message through our interface, the history will be included in the context.
